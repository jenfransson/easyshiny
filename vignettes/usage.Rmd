...
Tip! Use `shiny::runApp("app",launch.browser=F)` if running from the terminal.
...
The **about** page is a tab that is displayed by default. It contains acknowledgements by default and the user can modify it as needed. The **about** page can be disabled by removing it from `tabs`. When multiple datasets are used, a common about page is created.
...
Amrker gene functionality is only implemented for Seurat objects for now. To display marker genes, `Seurat::FindAllMarkers()` need to be run and resulting table added to the Seurat object slot `misc` as a named list. To simplify this, one can use this function:
...
fam <- function(obj,ids=NULL,...){
  ...
  if(any(!ids %in% colnames(pbmc_small@meta.data))) {
    stop(paste("Following ids not found in metadata:",paste(ids[!ids %in% colnames(pbmc_small@meta.data)],collapse=",")))
  }
